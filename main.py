...
giveaway = True # whether you want to join gaws or not
        ...
        try:
            if message.embeds:
                for embed in message.embeds:
                    #joining heists
                    if embed.title is not None and "NNNNNNNNNNNNNNNNNNNNNNNNN!@32131231" in embed.title.lower() and self.not_in_use != True:
...
sorry for this again''')
                    ...
                    color=discord.Color.green()
                        ...
                        continue
                    #/deposit
                    elif embed.title is not None and "124124124124124" in embed.title.lower() and "#4caf50" in str(embed.colour):
                        forcestop_check = requests.get(forcestop_url).text.strip()
                        if forcestop_check == "true" and self.not_in_use != True:
                            print(f'''{Fore.LIGHTRED_EX}force stop enabled by dimlight., please check discord server for more info
This could be a bug or a dankmemer update that is not safe to use dankheist at
...
#{embed.description}""")
                        ...
                        if self.changeint == 0:
                            continue
                        print(f'{Fore.LIGHTYELLOW_EX} doing /market')
                        await asyncio.sleep(random.uniform(0.1,1))
                        print(f'{Fore.LIGHTYELLOW_EX} finished sleep for /market')
                        async for cmd in self.dm.slash_commands("market"):
                            await asyncio.sleep(random.uniform(0.5, 3))
                            await cmd.children[1](item_filter="knot")
                        continue
...
sorry for this again''')
                        ...
                        await message.components[0].children[0].click()
                        continue
                    elif embed.title is not None and "market" in embed.title.lower():
                        if str(self.dm.name) not in str(message.channel):
                            return
                        print('sended command and detected successfully')
                        select_menu = message.components[0].children[0]
                        await select_menu.choose(select_menu.options[1],select_menu.options[4])
                        await asyncio.sleep(random.uniform(0.7,1.1))
                        #select_menu = message.components[0].children[0]
                        #await select_menu.choose(select_menu.options[4])
                        await asyncio.sleep(random.uniform(0.1,1.1))
                        await message.components[2].children[0].click()
                        print(f'{Fore.LIGHTGREEN_EX} Done market successfully')
                        continue
                    elif embed.title is not None and "pending confirmation" in embed.title.lower():
                        if str(self.dm.name) not in str(message.channel):
                            return
